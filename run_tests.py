...
def run_memory_operations_tests():
    """Run the memory operations tests."""
    print_header("Running Memory Operations Tests")

    try:
        result = subprocess.run(
            [sys.executable, "test_memory_operations.py"],
            capture_output=True,
            text=True
        )
        ...
        print(f"Error running memory operations tests: {str(e)}")
        return False
...
def run_all_tests():
    ...
    memory_tests_success = run_memory_operations_tests()
    ...
    print(f"Memory Operations Tests: {'PASSED' if memory_tests_success else 'FAILED'}")
    print(f"Overall: {'PASSED' if (http_api_success and mcp_bridge_success and data_tests_success and comment_tests_success and memory_tests_success) else 'FAILED'}")

    return http_api_success and mcp_bridge_success and data_tests_success and comment_tests_success and memory_tests_success
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--http":
            ...
            # Run only the memory operations tests
            success = run_memory_operations_tests()
        else:
            print(f"Unknown argument: {sys.argv[1]}")
            print("Usage: python run_tests.py [--http|--mcp|--data|--comments|--memory]")
            sys.exit(1)
    else:
        # Run all tests
