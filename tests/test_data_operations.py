...
def find_valid_addresses(port=GHIDRA_PORT) -> list:
    ...
    try:
        response = requests.get(f"http://localhost:{port}/memory/blocks")
        memory_info = json.loads(response.text)

        memory_blocks = memory_info.get("result", [])
        valid_addresses = []

        # First try to find an initialized RAM block
        for block in memory_blocks:
            if "start" in block and "name" in block and "RAM" in block["name"].upper() and block.get("isInitialized", False):
                addr_base = int(block["start"], 16)
                # Use addresses further in the block to avoid conflicts with instructions
                for i in range(10):
                    valid_addresses.append(f"{addr_base + 0x1000 + i*4:08x}")
                return valid_addresses

        # If no initialized RAM blocks, try any initialized block
        for block in memory_blocks:
            if "start" in block and block.get("isInitialized", False):
                addr_base = int(block["start"], 16)
                for i in range(10):
                    valid_addresses.append(f"{addr_base + 0x1000 + i*4:08x}")
                return valid_addresses
    ...
    return ["00001000", "00001004", "00001008", "0000100c", "00001010"]

def create_test_memory_block(port=GHIDRA_PORT) -> str:
    """Create a test memory block for data operations testing."""
    try:
        ...
        for block in memory_blocks:
            if "end" in block:
                addr = int(block["end"], 16)
                if addr > max_address:
                    max_address = addr

        # Create a new block after the last one
        test_address = f"{max_address + 0x1000:08x}"

        response = requests.post(
            f"http://localhost:{port}/memory/blocks",
            json={
        ...
        if response.status_code in [200, 201]:
            data = json.loads(response.text)
            if data.get("success", False):
                logger.info(f"Created test memory block at {test_address}")
                return test_address

        logger.error("Failed to create test memory block")
        return None
    except Exception as e:
        logger.error(f"Error creating test memory block: {e}")
        return None

def cleanup_test_memory_block(port=GHIDRA_PORT):
    """Clean up the test memory block."""
    try:
        response = requests.get(f"http://localhost:{port}/memory/blocks")
        memory_info = json.loads(response.text)
        ...
        memory_blocks = memory_info.get("result", [])
        for block in memory_blocks:
            if block.get("name") == "test_data_block":
                response = requests.delete(
                    f"http://localhost:{port}/memory/blocks",
                    json={"name": "test_data_block"}
                )
                logger.info(f"Cleaned up test memory block")
                break
    except Exception as e:
        logger.warning(f"Error cleaning up test memory block: {e}")
...
def test_http_data_create():
    ...
    test_block_address = create_test_memory_block()
    if not test_block_address:
        logger.error("Failed to create test memory block")
        return False
    ...
    try:
        ...
        for i, data_type in enumerate(types_to_try):
            # Calculate address within the test block
            address = f"{int(test_block_address, 16) + i*4:08x}"
...
def test_http_data_rename():
    ...
    try:
        test_address = test_block_address
...
def test_http_combined_operations():
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Error in combined operations test: {e}")
            return False
    finally:
        cleanup_test_memory_block()
